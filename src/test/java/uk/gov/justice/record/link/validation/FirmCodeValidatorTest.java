...
public class FirmCodeValidatorTest {
    private static final String VALID_LOGIN_ID = "valid_login_id";
    ...
    class IsValid {
        ...
        @Test
        void validFirmId() {
            when(mockCcmsUserRepository.existsByLoginIdAndFirmCode(eq(VALID_LOGIN_ID), eq(VALID_FIRM_CODE))).thenReturn(true);
            var userTransferRequest = new UserTransferRequest();
            userTransferRequest.setOldLogin(VALID_LOGIN_ID);
            userTransferRequest.setFirmCode(VALID_FIRM_CODE);

            var actualResult = firmIdValidator.isValid(userTransferRequest, mockConstraintValidatorContext);

            verify(mockCcmsUserRepository).existsByLoginIdAndFirmCode(loginIdCaptor.capture(), firmIdCaptor.capture());

            assertThat(actualResult).isTrue();
            assertThat(firmIdCaptor.getValue()).isEqualTo(VALID_FIRM_CODE);
            assertThat(loginIdCaptor.getValue()).isEqualTo(VALID_LOGIN_ID);
        ...
        @Test
        void invalidFirmId() {
            when(mockCcmsUserRepository.existsByLoginIdAndFirmCode(eq(VALID_LOGIN_ID), eq(INVALID_FIRM_CODE))).thenReturn(false);
            var userTransferRequest = new UserTransferRequest();
            userTransferRequest.setOldLogin(VALID_LOGIN_ID);
            userTransferRequest.setFirmCode(INVALID_FIRM_CODE);
